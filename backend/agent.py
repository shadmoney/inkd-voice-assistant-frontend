...
class LangGraphLLM(llm.LLM):
    def __init__(
        ...
        self.thread_id = str(id(self))  # Generate thread ID once for the entire conversation
...
class LangGraphStream(llm.LLMStream):
    ...
    async def _main_task(self) -> None:
        ...
        try:
            # Process through graph using the LLM's thread ID
            result = self.graph_app.invoke(
                {"messages": messages},
                {"configurable": {"thread_id": self._llm.thread_id}}
            )
            ...
            self._event_ch.send_nowait(
                ChatChunk(
                    request_id=self._llm.thread_id,  # Use same thread ID for request
                    choices=[
                        Choice(
                            delta=ChoiceDelta(
                                role="assistant",
    ...
    async def _run(self) -> None:
        """Run the stream processing. Required abstract method from parent class."""
